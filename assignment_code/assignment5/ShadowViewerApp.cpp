...
void ShadowViewerApp::SetupScene() {
  ...
  std::vector<MeshGroup> groups = mesh_data.groups;
  for (MeshGroup group : groups){
    std::unique_ptr<GLOO::SceneNode> group_node = make_unique<SceneNode>(); // Create the group node

    group_node->CreateComponent<ShadingComponent>(shader); // Set the shader

    RenderingComponent& rendering = group_node->CreateComponent<RenderingComponent>(vertex_obj); // Set the RenderingComponent
    size_t start_face_index = group.start_face_index;
    size_t num_indices = group.num_indices;
    rendering.SetDrawRange(start_face_index, num_indices);

    group_node->CreateComponent<MaterialComponent>(group.material); // Set the material

    root.AddChild(std::move(group_node)); // Add to tree
  }
